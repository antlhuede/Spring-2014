...
public sealed class UnmanagedLibrary : IDisposable
{
	...
	public void Reload()
	{
		...
		m_hLibrary = NativeMethods.LoadLibrary(m_DllFileName);
		if (m_hLibrary.IsInvalid)
		{
			int hr = Marshal.GetHRForLastWin32Error();
			Marshal.ThrowExceptionForHR(hr);
	...
	public bool HasUnmanagedFunction(string functionName)
	{
		IntPtr functionPointer = NativeMethods.GetProcAddress(m_hLibrary, functionName);
